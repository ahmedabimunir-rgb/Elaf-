...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBackToCustomerView }) => {
  ...
  const showToast = (msg: string) => {
    ...
    setTimeout(() => {
      setFeedbackToast((curr) => (curr === msg ? null : curr));
    }, 3500);
  };

  // Dispatches audible chime + visual toast + row glow + title blinking + optional desktop notification
  const triggerOrderAlert = (newOrder: Order) => {
    // 1. Audio Alert
    SoundService.playNewOrderChime();

    // 2. Visual Toast Alert (stack up to 3)
    const notifId = `notif-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`;
    setNotifications((prev) => [
      { id: notifId, order: newOrder, timestamp: Date.now() },
      ...prev.filter((n) => n.order.id !== newOrder.id).slice(0, 2),
    ]);

    // 3. Highlight order row in lists
    setHighlightedOrderId(newOrder.id);
    setTimeout(() => {
      setHighlightedOrderId((curr) => (curr === newOrder.id ? null : curr));
    }, 20000);

    // 4. Flash browser document title
    if (typeof document !== 'undefined') {
      if (titleIntervalRef.current) clearInterval(titleIntervalRef.current);
      let isFlashing = true;
      let ticks = 0;
      const baseTitle = 'Elaf Restaurant Portal';
      titleIntervalRef.current = window.setInterval(() => {
        ...
        ticks++;
        if (ticks >= 14 && titleIntervalRef.current) {
    ...
    // 5. Desktop Push Notification (if permission granted)
    try {
      if (
        typeof window !== 'undefined' &&
        'Notification' in window &&
        ...
        new Notification(`🔔 New Order: ${newOrder.orderNumber}`, {
          body: `${newOrder.customerName} ordered ${newOrder.items.length} item(s) • ${formatPrice(newOrder.total)}`,
          icon: '/favicon.ico',
        });
      }
    } catch {}
  ...
  // Subscribe to real-time incoming orders & heartbeat poller
  useEffect(() => {
    knownOrderIdsRef.current = new Set(orders.map((o) => o.id));

    // Instant event subscription (same tab & cross-tab)
    const unsubscribe = StoreService.subscribeToNewOrders((newOrder) => {
    ...
    // Fallback heartbeat poller every 3.5s to ensure no order is ever missed
    const pollInterval = window.setInterval(() => {
      const latestOrders = StoreService.getOrders();
    ...
    return () => {
      ...
      if (typeof document !== 'undefined') {
        document.title = 'Elaf Restaurant Portal';
      }
    };
  }, []);
  ...
  const toggleSound = () => {
    ...
    SoundService.setSoundMuted(next);
    setIsSoundMuted(next);
    if (!next) {
      SoundService.playConfirmChime();
      showToast('🔊 Order chime sound alerts enabled');
    } else {
      showToast('🔇 Order chime sound alerts muted');
    }
  };
  ...
  const handleTestChime = () => {
    ...
    SoundService.playNewOrderChime();
    showToast('🔔 Playing kitchen chime audio test...');
  };

  const handleSimulateOrder = () => {
    SoundService.unlockAudio();
    const testOrder = StoreService.simulateTestOrder();
    showToast(`⚡ Generated live test order ${testOrder.orderNumber}!`);
  };
  ...
  const handleRequestDesktopPermission = async () => {
    if (typeof window !== 'undefined' && 'Notification' in window) {
      ...
      if (res === 'granted') {
        ...
        try {
          new Notification('🔔 Elaf Kitchen Notifications Active', {
            body: 'You will receive desktop alerts when customers submit new orders.',
          });
        } catch {}
      } else {
        showToast('Desktop notifications not permitted in browser.');
      }
    }
  };
  ...
  const handleViewNotifiedOrder = (order: Order, notifId: string) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200 flex flex-col relative">
      {/* ================= REAL-TIME FLOATING ORDER ALERTS (TOP-RIGHT) ================= */}
      <div className="fixed top-20 right-4 sm:right-6 z-50 flex flex-col gap-3 max-w-sm w-full pointer-events-none">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className="pointer-events-auto bg-zinc-950/95 border-2 border-amber-500/90 rounded-2xl p-4 shadow-2xl shadow-amber-950/70 backdrop-blur-md transition-all duration-300 transform translate-y-0"
          >
            ...
            <div className="mt-2.5 space-y-1">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-amber-400 text-sm">
                  {formatPrice(notif.order.total)}
                </span>
              </div>
              <p className="text-xs text-zinc-300 font-medium truncate">
                {notif.order.customerName} • {notif.order.customerPhone}
              </p>
              <div className="flex items-center gap-2 text-[11px] text-zinc-400 pt-0.5">
                <span className="px-2 py-0.5 rounded bg-zinc-900 border border-zinc-800 font-semibold text-zinc-300">
                  {notif.order.orderType === 'DELIVERY'
                    ? '🛵 Delivery'
                    : notif.order.orderType === 'DINE_IN'
                    ? `🍽️ Dine-In (${notif.order.tableNumber || 'Table'})`
                    : '🥡 Pickup'}
                </span>
                <span>{notif.order.items.length} item(s)</span>
              </div>
              {notif.order.deliveryAddress && notif.order.orderType === 'DELIVERY' && (
                <p className="text-[11px] text-zinc-400 truncate flex items-center gap-1">
                  <MapPin className="w-3 h-3 text-rose-500 shrink-0" />
                  {notif.order.deliveryAddress}
                </p>
              )}
            </div>
            ...
            <div className="mt-3 flex items-center gap-2">
              <button
                onClick={() => handleViewNotifiedOrder(notif.order, notif.id)}
                className="flex-1 py-1.5 px-3 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-xs rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow-md"
              >
              ...
              <button
                onClick={() => dismissNotification(notif.id)}
                className="py-1.5 px-2.5 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white text-xs font-semibold rounded-xl border border-zinc-800 transition-colors"
              >
                Dismiss
              </button>
            </div>
      ...
      {/* Real-time Order Notification & Sound Control Sub-Toolbar */}
      <div className="bg-zinc-900/90 border-b border-zinc-800/80 px-6 py-2 flex flex-wrap items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-zinc-300 font-medium">
            ...
            <span className="text-zinc-300 text-xs">Real-Time Kitchen Radar Active</span>
          </div>

          {feedbackToast && (
            <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 text-[11px] font-semibold animate-pulse">
              {feedbackToast}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-semibold text-xs transition-all border ${
              !isSoundMuted
                ...
                : 'bg-zinc-800/90 border-zinc-700 text-zinc-400 hover:text-zinc-200'
            }`}
            title={!isSoundMuted ? 'Mute order notification sound' : 'Enable order notification sound'}
          >
            {!isSoundMuted ? (
              <>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-xl font-semibold text-xs border border-zinc-700 transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-950/70 hover:bg-rose-900 border border-rose-800 text-rose-300 rounded-xl font-semibold text-xs transition-colors"
            title="Simulate a real-time incoming customer order"
          >
          ...
          {!hasDesktopPermission && typeof window !== 'undefined' && 'Notification' in window && (
            <button
              onClick={handleRequestDesktopPermission}
              className="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-blue-950/70 hover:bg-blue-900 border border-blue-800 text-blue-300 rounded-xl font-semibold text-xs transition-colors"
              title="Allow browser notifications for new orders"
            >
      ...
      <div className="bg-zinc-950 border-b border-zinc-800/80 px-6 overflow-x-auto no-scrollbar">
        <div className="flex items-center gap-2 py-2.5 min-w-max">
          ...
          >
            ...
            {pendingCount > 0 && (
              <span className="px-1.5 py-0.2 rounded-full text-[10px] font-black bg-amber-400 text-zinc-950 animate-pulse">
                {pendingCount}
              </span>
            )}
          </button>
                    ...
                    {orders.slice(0, 5).map((ord) => {
                      ...
                      <tr
                        ...
                        className={`transition-colors ${
                          isHighlighted
                            ? 'bg-amber-500/20 ring-2 ring-amber-400/80'
                      ...
                      >
                        <td className="py-3 px-4 font-mono font-bold text-rose-400">
                          <div className="flex items-center gap-1.5">
                            ...
                            {isHighlighted && (
                              <span className="px-1.5 py-0.5 rounded text-[9px] font-black bg-amber-400 text-zinc-950 uppercase tracking-wider animate-pulse">
                                JUST IN
                              </span>
                            )}
                          </div>
                ...
                >
                  <div className="space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="font-sans font-bold text-amber-400 text-sm whitespace-nowrap">
                        {dish.price.toLocaleString()} ETB
                      </span>
                    </div>
                    <p className="text-xs text-zinc-400 line-clamp-2 leading-relaxed">{dish.description}</p>
                  </div>
